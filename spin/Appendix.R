#'---
#' title: 'Supporting material for "The Devil is in the exclusions: Inter-measure agreement among metrics for low-value imaging for low back pain."'
#' date: 'May 21, 2019'
...
#' exception of the specific exclusions listed in table 2. 
#' 
#' This file also contains the code used to compute agreement statistics from
#' the counts provided. To view this code, use the `Code` buttons along the right
#' side of the page or choose "Show All Code" on the upper right.
#' The source code for the html file you are reading is aviable as an
#'  executable R script, `Appendix.R`.
#' 
#' ## Tables
...
num_tab[order(C_im, H_im, S_im),
  ...
  kableExtra::kable_styling("striped", full_width = TRUE) %>%
  kableExtra::add_header_above(c("Low-value LBP Imaging by Measure" = 3, 
                                 " "= 1, " "= 1))
...
#' in a particular component as defined by measure $k$ $(k = 1, \cdots, K)$, then
#' 
#' $$
